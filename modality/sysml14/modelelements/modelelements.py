...
from modality.pyuml2.types import Integer, String
from . import modelelements_mixins as _user_module
...
class Conform(_user_module.ConformMixin, EObject, metaclass=MetaEClass):
    ...
    The view conforms to the specified rules and conventions detailed in
    the viewpoint. Conform is a specialization of the UML dependency, and
    as with other dependencies the arrow direction points from the
    (client/source) to the (supplier/target).
    """
...
class ElementGroup(_user_module.ElementGroupMixin, EObject, metaclass=MetaEClass):

    _criterion = EAttribute(
        eType=String, derived=True, changeable=False, name="criterion", transient=True
    )
    name = EAttribute(eType=String, derived=False, changeable=True)
    _size = EAttribute(
        eType=Integer, derived=True, changeable=False, name="size", transient=True
    )
...
class Problem(_user_module.ProblemMixin, EObject, metaclass=MetaEClass):
    ...
    model elements to satisfy a requirement or need, or other undesired
    outcome. It may be used to capture problems identified during analysis,
    design, verification, or manufacture and associate the problem with
    the relevant model elements. Problem is a stereotype of comment and may
...
class Rationale(_user_module.RationaleMixin, EObject, metaclass=MetaEClass):
    ...
    design, and other decisions. A Rationale can be attached to any model
    element including relationships. It allows the user, for example, to
    specify a rationale that may reference more detailed documentation such
    as a trade study or analysis report. Rationale is a stereotype of comment
    and may be attached to any other model element in the same manner as a comment.
    """
...
class Stakeholder(_user_module.StakeholderMixin, EObject, metaclass=MetaEClass):
...
class View(_user_module.ViewMixin, EObject, metaclass=MetaEClass):
    ...
    stakeholder = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedStakeholder,
    )
...
class Viewpoint(_user_module.ViewpointMixin, EObject, metaclass=MetaEClass):
    ...
    presentation = EAttribute(eType=String, derived=False, changeable=True, upper=-1)
    ...
    method = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedMethod,
    )
    stakeholder = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
