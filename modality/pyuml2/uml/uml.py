...
import modality.pyuml2.uml_mixins as _user_module
...
VisibilityKind = EEnum(
    "VisibilityKind", literals=["public", "private", "protected", "package"]
)

ParameterDirectionKind = EEnum(
    "ParameterDirectionKind", literals=["in_", "inout", "out", "return_"]
)

ParameterEffectKind = EEnum(
    "ParameterEffectKind", literals=["create", "read", "update", "delete"]
)

CallConcurrencyKind = EEnum(
    "CallConcurrencyKind", literals=["sequential", "guarded", "concurrent"]
)

TransitionKind = EEnum("TransitionKind", literals=["internal", "local", "external"])

PseudostateKind = EEnum(
    "PseudostateKind",
    literals=[
        "initial",
        "deepHistory",
...
AggregationKind = EEnum("AggregationKind", literals=["none", "shared", "composite"])

ConnectorKind = EEnum("ConnectorKind", literals=["assembly", "delegation"])

ObjectNodeOrderingKind = EEnum(
    "ObjectNodeOrderingKind", literals=["unordered", "ordered", "LIFO", "FIFO"]
)

ExpansionKind = EEnum("ExpansionKind", literals=["parallel", "iterative", "stream"])

MessageKind = EEnum("MessageKind", literals=["complete", "lost", "found", "unknown"])
...
@abstract
@EMetaclass
class ActivityContent(_user_module.ActivityContentMixin, EObject):
...
class Element(_user_module.ElementMixin, EModelElement):
    ...
    ownedElement = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOwnedelement,
    )
    _owner = EReference(
        ordered=False,
        unique=True,
...
class NamedElement(_user_module.NamedElementMixin, Element):
    ...
    visibility = EAttribute(eType=VisibilityKind, derived=False, changeable=True)
    clientDependency = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedClientdependency,
    )
...
class ParameterableElement(_user_module.ParameterableElementMixin, Element):
    ...
    TemplateParameter for a template, or specified as an actual parameter
...
class TemplateSignature(_user_module.TemplateSignatureMixin, Element):
    ...
    template = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Relationship(_user_module.RelationshipMixin, Element):
    ...
    relatedElement = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedRelatedelement,
    )
...
class TemplateParameterSubstitution(
    ...
    actual = EReference(ordered=False, unique=True, containment=False, derived=False)
    formal = EReference(ordered=False, unique=True, containment=False, derived=False)
    ownedActual = EReference(
        ordered=False, unique=True, containment=True, derived=False
    )
    templateBinding = EReference(
        ordered=False, unique=True, containment=False, derived=False
...
class MultiplicityElement(_user_module.MultiplicityElementMixin, Element):
    ...
    lowerValue = EReference(ordered=False, unique=True, containment=True, derived=False)
    upperValue = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class LinkEndData(_user_module.LinkEndDataMixin, Element):
    ...
    objects and qualifier values, if any. A LinkEndData instance provides these values
    for a single Association end.
    <p>From package UML::Actions.</p>"""

    end = EReference(ordered=False, unique=True, containment=False, derived=False)
    qualifier = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    value = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class QualifierValue(_user_module.QualifierValueMixin, Element):
    ...
    qualifier = EReference(ordered=False, unique=True, containment=False, derived=False)
    value = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Clause(_user_module.ClauseMixin, Element):
    ...
    including a test and a body section. The body section is executed only if
    (but not necessarily if) the test section evaluates to true.
    <p>From package UML::Actions.</p>"""

    body = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
    bodyOutput = EReference(
        ordered=True, unique=True, containment=False, derived=False, upper=-1
    )
    decider = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Namespace(_user_module.NamespaceMixin, NamedElement):
    ...
    ownedMember = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOwnedmember,
    )
    importedMember = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedImportedmember,
    )
    member = EReference(
        ordered=False,
        unique=True,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedMember,
    )
...
@abstract
class DirectedRelationship(_user_module.DirectedRelationshipMixin, Relationship):
    ...
    source = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedSource,
    )
    target = EReference(
        ordered=False,
        unique=True,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedTarget,
    )
...
class ConnectorEnd(_user_module.ConnectorEndMixin, MultiplicityElement):
    ...
    role = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class DeploymentTarget(_user_module.DeploymentTargetMixin, NamedElement):
    ...
    deployedElement = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedDeployedelement,
    )
    deployment = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
...
class DeployedArtifact(_user_module.DeployedArtifactMixin, NamedElement):
    """A deployed artifact is an artifact or artifact instance that has been
    deployed to a deployment target.
    <p>From package UML::Deployments.</p>"""
...
class RedefinableElement(_user_module.RedefinableElementMixin, NamedElement):
    ...
    Classifier, can be redefined more specifically or differently in the context
    of another Classifier that specializes (directly or indirectly) the context
    Classifier.
    <p>From package UML::Classification.</p>"""
    ...
    redefinedElement = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedRedefinedelement,
    )
    redefinitionContext = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedRedefinitioncontext,
    )
...
class Vertex(_user_module.VertexMixin, NamedElement):
    ...
    container = EReference(ordered=False, unique=True, containment=False, derived=False)
    incoming = EReference(
        ordered=False,
        unique=True,
        containment=False,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedIncoming,
    )
    outgoing = EReference(
        ordered=False,
        unique=True,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOutgoing,
    )
...
class Trigger(_user_module.TriggerMixin, NamedElement):
    ...
    trigger an effect in a Behavior. A Trigger may be qualified by the Port on
    which the Event occurred.
    <p>From package UML::CommonBehavior.</p>"""

    event = EReference(ordered=False, unique=True, containment=False, derived=False)
    port = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
...
class CollaborationUse(_user_module.CollaborationUseMixin, NamedElement):
    """A CollaborationUse is used to specify the application of a pattern
    specified by a Collaboration to a specific situation.
    <p>From package UML::StructuredClassifiers.</p>"""

    roleBinding = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    type = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class LinkEndCreationData(_user_module.LinkEndCreationDataMixin, LinkEndData):
    ...
    insertAt = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class LinkEndDestructionData(_user_module.LinkEndDestructionDataMixin, LinkEndData):
    ...
    destroyAt = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Message(_user_module.MessageMixin, NamedElement):
    ...
    _messageKind = EAttribute(
        ...
        changeable=False,
        name="messageKind",
        default_value=MessageKind.unknown,
        transient=True,
    )
    ...
    sendEvent = EReference(ordered=False, unique=True, containment=False, derived=False)
    signature = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class InteractionFragment(_user_module.InteractionFragmentMixin, NamedElement):
    """InteractionFragment is an abstract notion of the most general interaction
     unit. An InteractionFragment is a piece of an Interaction. Each
     InteractionFragment is conceptually like an Interaction by itself.
    <p>From package UML::Interactions.</p>"""
...
class Lifeline(_user_module.LifelineMixin, NamedElement):
    ...
    While parts and structural features may have multiplicity greater than 1,
    ...
    represents = EReference(
        ordered=False, unique=True, containment=False, derived=False
    )
    selector = EReference(ordered=False, unique=True, containment=True, derived=False)
    coveredBy = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
...
class MessageEnd(_user_module.MessageEndMixin, NamedElement):
    ...
    message = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class GeneralOrdering(_user_module.GeneralOrderingMixin, NamedElement):
    ...
    must occur before the other in a valid trace. This mechanism provides the
    ability to define partial orders of OccurrenceSpecifications that may
    otherwise not have a specified order.
    <p>From package UML::Interactions.</p>"""

    after = EReference(ordered=False, unique=True, containment=False, derived=False)
    before = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Feature(_user_module.FeatureMixin, RedefinableElement):
    ...
    featuringClassifier = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedFeaturingclassifier,
    )
...
class Pseudostate(_user_module.PseudostateMixin, Vertex):
    ...
    transient Vertices in the StateMachine graph. A StateMachine instance
    never comes to rest in a Pseudostate, instead, it will exit and enter
    ...
    state = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class ConnectionPointReference(_user_module.ConnectionPointReferenceMixin, Vertex):
    """A ConnectionPointReference represents a usage (as part of a submachine
    State) of an entry/exit point Pseudostate defined in the StateMachine
...
class ProtocolConformance(_user_module.ProtocolConformanceMixin, DirectedRelationship):
...
class ProfileApplication(_user_module.ProfileApplicationMixin, DirectedRelationship):
...
class ElementImport(_user_module.ElementImportMixin, DirectedRelationship):
    """An ElementImport identifies a NamedElement in a Namespace other than
    the one that owns that NamedElement and allows the NamedElement to be
    referenced using an unqualified name in the Namespace owning the ElementImport.
    <p>From package UML::CommonStructure.</p>"""
...
class Generalization(_user_module.GeneralizationMixin, DirectedRelationship):
    ...
    Classifier is also an instance of the general Classifier. The specific
    Classifier inherits the features of the more general Classifier. A
...
class ExtensionPoint(_user_module.ExtensionPointMixin, RedefinableElement):
    ...
    useCase = EReference(ordered=False, unique=True, containment=False, derived=False)
...
@abstract
class ActivityGroup(_user_module.ActivityGroupMixin, NamedElement, ActivityContent):
    """ActivityGroup is an abstract class for defining sets of ActivityNodes
    and ActivityEdges in an Activity.
    <p>From package UML::Activities.</p>"""
    ...
    containedEdge = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedContainededge,
    )
    containedNode = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedContainednode,
    )
    ...
    subgroup = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedSubgroup,
    )
...
class ActivityEdge(_user_module.ActivityEdgeMixin, RedefinableElement):
    """An ActivityEdge is an abstract class for directed connections between
    two ActivityNodes.
    <p>From package UML::Activities.</p>"""

    activity = EReference(ordered=False, unique=True, containment=False, derived=False)
    guard = EReference(ordered=False, unique=True, containment=True, derived=False)
    inPartition = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
    interrupts = EReference(
        ordered=False, unique=True, containment=False, derived=False
    )
    ...
    weight = EReference(ordered=False, unique=True, containment=True, derived=False)
    inGroup = EReference(
        ordered=False,
        unique=True,
        containment=False,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedIngroup,
    )
...
class InteractionUse(_user_module.InteractionUseMixin, InteractionFragment):
    """An InteractionUse refers to an Interaction. The InteractionUse is a
    shorthand for copying the contents of the referenced Interaction where
    the InteractionUse is. To be accurate the copying must take into account
    substituting parameters with arguments and connect the formal Gates with
    the actual ones.
    <p>From package UML::Interactions.</p>"""

    actualGate = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    argument = EReference(
        ordered=True, unique=True, containment=True, derived=False, upper=-1
    )
    refersTo = EReference(ordered=False, unique=True, containment=False, derived=False)
    returnValue = EReference(
        ordered=False, unique=True, containment=True, derived=False
    )
...
class Gate(_user_module.GateMixin, MessageEnd):
    ...
    InteractionFragment with another Message which has a MessageEnd
...
class ExecutionSpecification(
    ...
    of Behavior or Action within the Lifeline. The duration of an
    ExecutionSpecification is represented by two OccurrenceSpecifications,
    the start OccurrenceSpecification and the finish OccurrenceSpecification.
    <p>From package UML::Interactions.</p>"""

    finish = EReference(ordered=False, unique=True, containment=False, derived=False)
    start = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class CombinedFragment(_user_module.CombinedFragmentMixin, InteractionFragment):
    ...
    InteractionOperands. Through the use of CombinedFragments the user will
...
class StateInvariant(_user_module.StateInvariantMixin, InteractionFragment):
    """A StateInvariant is a runtime constraint on the participants of the
    ...
    external States, and so on. A StateInvariant is an InteractionFragment
    and it is placed on a Lifeline.
    <p>From package UML::Interactions.</p>"""

    invariant = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ConnectableElement(
    ...
    instances that play roles of a StructuredClassifier. ConnectableElements
    may be joined by attached Connectors and specify configurations of
...
class Constraint(_user_module.ConstraintMixin, PackageableElement):
    ...
    declaring some of the semantics of an Element or set of Elements.
    <p>From package UML::CommonStructure.</p>"""

    constrainedElement = EReference(
        ordered=True, unique=True, containment=False, derived=False, upper=-1
    )
    context = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Region(_user_module.RegionMixin, Namespace, RedefinableElement):
    """A Region is a top-level part of a StateMachine or a composite State,
    that serves as a container for the Vertices and Transitions of the
    StateMachine. A StateMachine or composite State may contain multiple
...
class Transition(_user_module.TransitionMixin, Namespace, RedefinableElement):
    """A Transition represents an arc between exactly one source Vertex and
    exactly one Target vertex (the source and targets may be the same Vertex).
    It may form part of a compound transition, which takes the StateMachine
    from one steady State configuration to another, representing the full
    response of the StateMachine to an occurrence of an Event that triggered it.
    <p>From package UML::StateMachines.</p>"""
    ...
    effect = EReference(ordered=False, unique=True, containment=True, derived=False)
    guard = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Connector(_user_module.ConnectorMixin, Feature):
    """A Connector specifies links that enables communication between two or
    more instances. In contrast to Associations, which specify links between
    any instance of the associated Classifiers, Connectors specify links
    between instances playing the connected parts only.
    <p>From package UML::StructuredClassifiers.</p>"""

    _kind = EAttribute(
        eType=ConnectorKind, derived=True, changeable=False, name="kind", transient=True
    )
    contract = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
...
class GeneralizationSet(_user_module.GeneralizationSetMixin, PackageableElement):
    """A GeneralizationSet is a PackageableElement whose instances represent
    sets of Generalization relationships.
    <p>From package UML::Classification.</p>"""

    isCovering = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isDisjoint = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    powertype = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class RedefinableTemplateSignature(
    _user_module.RedefinableTemplateSignatureMixin,
    RedefinableElement,
    TemplateSignature,
):
    ...
    inheritedParameter = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedInheritedparameter,
    )
    classifier = EReference(
        ordered=False, unique=True, containment=False, derived=False, transient=True
    )
...
class Extend(_user_module.ExtendMixin, NamedElement, DirectedRelationship):
    """A relationship from an extending UseCase to an extended UseCase that
    specifies how and when the behavior defined in the extending UseCase can
    be inserted into the behavior defined in the extended UseCase.
    <p>From package UML::UseCases.</p>"""

    condition = EReference(ordered=False, unique=True, containment=True, derived=False)
    ...
    extension = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Include(_user_module.IncludeMixin, NamedElement, DirectedRelationship):
    ...
    addition = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class ActivityPartition(_user_module.ActivityPartitionMixin, ActivityGroup):
    ...
    ActivityNodes that have some characteristic in common.
    <p>From package UML::Activities.</p>"""

    isDimension = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isExternal = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    node = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
...
@abstract
class ActivityNode(_user_module.ActivityNodeMixin, RedefinableElement, ActivityContent):
...
class ObjectFlow(_user_module.ObjectFlowMixin, ActivityEdge):
    ...
    isMulticast = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isMultireceive = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    selection = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Observation(_user_module.ObservationMixin, PackageableElement):
    """Observation specifies a value determined by observing an event or
    events that occur relative to other model Elements.
...
class InteractionOperand(
    ...
    """An InteractionOperand is contained in a CombinedFragment. An
...
class ActionExecutionSpecification(
    ...
    action = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class BehaviorExecutionSpecification(
    ...
    behavior = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class ExecutionOccurrenceSpecification(
    _user_module.ExecutionOccurrenceSpecificationMixin, OccurrenceSpecification
):
    """An ExecutionOccurrenceSpecification represents moments in time at which
    Actions or Behaviors start or finish.
    <p>From package UML::Interactions.</p>"""

    execution = EReference(ordered=False, unique=True, containment=False, derived=False)
...
@abstract
class BehavioralFeature(_user_module.BehavioralFeatureMixin, Namespace, Feature):
    """A BehavioralFeature is a feature of a Classifier that specifies an aspect
    of the behavior of its instances.  A BehavioralFeature is implemented
    (realized) by a Behavior. A BehavioralFeature specifies that a Classifier
...
class State(_user_module.StateMixin, Namespace, RedefinableElement, Vertex):
    ...
    doActivity = EReference(ordered=False, unique=True, containment=True, derived=False)
    entry = EReference(ordered=False, unique=True, containment=True, derived=False)
    exit = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ExecutableNode(_user_module.ExecutableNodeMixin, ActivityNode):
    ...
    may be controlled using ControlFlows and to which ExceptionHandlers may be
    attached.
    <p>From package UML::Activities.</p>"""
...
class TimeEvent(_user_module.TimeEventMixin, Event):
    ...
    isRelative = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    when = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class InteractionConstraint(_user_module.InteractionConstraintMixin, Constraint):
    """An InteractionConstraint is a Boolean expression that guards an operand
    in a CombinedFragment.
    <p>From package UML::Interactions.</p>"""

    maxint = EReference(ordered=False, unique=True, containment=True, derived=False)
    minint = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class MessageOccurrenceSpecification(
    _user_module.MessageOccurrenceSpecificationMixin,
...
class ProtocolTransition(_user_module.ProtocolTransitionMixin, Transition):
    ...
    of ProtocolStateMachines have the following information: a pre-condition (guard),
    a Trigger, and a post-condition. Every ProtocolTransition is associated with at
    most one BehavioralFeature belonging to the context Classifier of the
    ...
    referred = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedReferred,
    )
...
class DurationObservation(_user_module.DurationObservationMixin, Observation):
    ...
    firstEvent = EAttribute(eType=Boolean, derived=False, changeable=True, upper=-1)
...
class Package(
    _user_module.PackageMixin, Namespace, PackageableElement, TemplateableElement
):
    ...
    ParameterableElement to specify that a package can be used as a template and a
    ...
    nestedPackage = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedNestedpackage,
    )
    ...
    ownedStereotype = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOwnedstereotype,
    )
    ownedType = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOwnedtype,
    )
...
class Dependency(
    _user_module.DependencyMixin, PackageableElement, DirectedRelationship
):
    """A Dependency is a Relationship that signifies that a single model Element or
    a set of model Elements requires other model Elements for their specification
    or implementation. This means that the complete semantics of the client Element(s)
...
class OpaqueExpression(_user_module.OpaqueExpressionMixin, ValueSpecification):
    ...
    of a collection of values either in terms of a UML Behavior or based on a
    textual statement in a language other than UML
    <p>From package UML::Values.</p>"""

    body = EAttribute(eType=String, derived=False, changeable=True, upper=-1)
    language = EAttribute(eType=String, derived=False, changeable=True, upper=-1)
    behavior = EReference(ordered=False, unique=True, containment=False, derived=False)
    _result = EReference(
        ordered=False,
        unique=True,
...
class Parameter(_user_module.ParameterMixin, ConnectableElement, MultiplicityElement):
    """A Parameter is a specification of an argument used to pass information
    into or out of an invocation of a BehavioralFeature.  Parameters can be treated
    as ConnectableElements within Collaborations.
    <p>From package UML::Classification.</p>"""

    _default = EAttribute(
        eType=String, derived=True, changeable=True, name="default", transient=True
    )
    ...
    effect = EAttribute(eType=ParameterEffectKind, derived=False, changeable=True)
    isException = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isStream = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
...
class Reception(_user_module.ReceptionMixin, BehavioralFeature):
    ...
    signal = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class StructuralFeature(
    _user_module.StructuralFeatureMixin, Feature, TypedElement, MultiplicityElement
):
...
class InstanceSpecification(
    _user_module.InstanceSpecificationMixin,
    DeploymentTarget,
    PackageableElement,
    DeployedArtifact,
):
    ...
    in a Deployment relationship, in the case that it represents an instance of
    a Node. It can also act as a DeployedArtifact, if it represents an instance
    of an Artifact.
    <p>From package UML::Classification.</p>"""
...
class Expression(_user_module.ExpressionMixin, ValueSpecification):
    ...
    sequence of operands that are ValueSpecifications. It denotes a (possibly empty)
...
class Action(_user_module.ActionMixin, ExecutableNode):
    ...
    system. Actions provide the ExecutableNodes within Activities and may also be
    used within Interactions.
    <p>From package UML::Actions.</p>"""
    ...
    input = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedInput,
    )
    ...
    output = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOutput,
    )
...
class ObjectNode(_user_module.ObjectNodeMixin, ActivityNode, TypedElement):
    ...
    selection = EReference(ordered=False, unique=True, containment=False, derived=False)
    upperBound = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class Variable(_user_module.VariableMixin, ConnectableElement, MultiplicityElement):
    """A Variable is a ConnectableElement that may store values during the
    execution of an Activity. Reading and writing the values of a Variable
    provides an alternative means for passing data than the use of ObjectFlows.
    A Variable may be owned directly by an Activity, in which case it is accessible
    from anywhere within that activity, or it may be owned by a
    ...
    scope = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class JoinNode(_user_module.JoinNodeMixin, ControlNode):
    ...
    joinSpec = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class InstanceValue(_user_module.InstanceValueMixin, ValueSpecification):
    ...
    instance = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class AnyReceiveEvent(_user_module.AnyReceiveEventMixin, MessageEvent):
    """A trigger for an AnyReceiveEvent is triggered by the receipt of any
...
class CallEvent(_user_module.CallEventMixin, MessageEvent):
    """A CallEvent models the receipt by an object of a message invoking a
    call of an Operation.
    <p>From package UML::CommonBehavior.</p>"""

    operation = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class TimeExpression(_user_module.TimeExpressionMixin, ValueSpecification):
    ...
    expr = EReference(ordered=False, unique=True, containment=True, derived=False)
    observation = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
...
class InformationFlow(
    _user_module.InformationFlowMixin, PackageableElement, DirectedRelationship
):
    """InformationFlows describe circulation of information through a system
    in a general manner. They do not specify the nature of the information,
    mechanisms by which it is conveyed, sequences of exchange or any control
    conditions. During more detailed modeling, representation and realization
    links may be added to specify which model elements implement an InformationFlow
    and to show how information is conveyed.  InformationFlows require some kind
    of “information channel” for unidirectional transmission of information items
    from sources to targets.  They specify the information channel’s realizations,
    if any, and identify the information that flows along them.  Information
    moving along the information channel may be represented by abstract
    InformationItems and by concrete Classifiers.
...
class DestructionOccurrenceSpecification(
    _user_module.DestructionOccurrenceSpecificationMixin, MessageOccurrenceSpecification
):
...
class DurationConstraint(_user_module.DurationConstraintMixin, IntervalConstraint):
...
class Interval(_user_module.IntervalMixin, ValueSpecification):
    ...
    max = EReference(ordered=False, unique=True, containment=False, derived=False)
    min = EReference(ordered=False, unique=True, containment=False, derived=False)
...
@abstract
class LiteralSpecification(_user_module.LiteralSpecificationMixin, ValueSpecification):
...
class Deployment(_user_module.DeploymentMixin, Dependency):
    ...
    location = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class Abstraction(_user_module.AbstractionMixin, Dependency):
    ...
    mapping = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class EnumerationLiteral(_user_module.EnumerationLiteralMixin, InstanceSpecification):
    """An EnumerationLiteral is a user-defined data value for an Enumeration.
    <p>From package UML::SimpleClassifiers.</p>"""

    enumeration = EReference(
        ordered=False, unique=True, containment=False, derived=False
    )
...
class Model(_user_module.ModelMixin, Package):
    ...
    physical system, with a certain purpose. This purpose determines what is to
...
class ValueSpecificationAction(_user_module.ValueSpecificationActionMixin, Action):
    """A ValueSpecificationAction is an Action that evaluates a ValueSpecification
    and provides a result.
    <p>From package UML::Actions.</p>"""

    result = EReference(ordered=False, unique=True, containment=True, derived=False)
    value = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class VariableAction(_user_module.VariableActionMixin, Action):
    ...
    variable = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class LinkAction(_user_module.LinkActionMixin, Action):
    """LinkAction is an abstract class for all Actions that identify the links to
    be acted on using LinkEndData.
    <p>From package UML::Actions.</p>"""

    endData = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    inputValue = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
...
@abstract
class StructuralFeatureAction(_user_module.StructuralFeatureActionMixin, Action):
    """StructuralFeatureAction is an abstract class for all Actions that operate
    on StructuralFeatures.
    <p>From package UML::Actions.</p>"""

    object = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class AcceptEventAction(_user_module.AcceptEventActionMixin, Action):
    """An AcceptEventAction is an Action that waits for the occurrence of one or
...
class InvocationAction(_user_module.InvocationActionMixin, Action):
    ...
    onPort = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class CreateObjectAction(_user_module.CreateObjectActionMixin, Action):
    """A CreateObjectAction is an Action that creates an instance of the
...
class DestroyObjectAction(_user_module.DestroyObjectActionMixin, Action):
    ...
    target = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class RaiseExceptionAction(_user_module.RaiseExceptionActionMixin, Action):
    ...
    exception = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ReadExtentAction(_user_module.ReadExtentActionMixin, Action):
    """A ReadExtentAction is an Action that retrieves the current instances
    of a Classifier.
    <p>From package UML::Actions.</p>"""
...
class ReadLinkObjectEndAction(_user_module.ReadLinkObjectEndActionMixin, Action):
    """A ReadLinkObjectEndAction is an Action that retrieves an end object
    from a link object.
    <p>From package UML::Actions.</p>"""
...
class ReadSelfAction(_user_module.ReadSelfActionMixin, Action):
    ...
    Behavior execution within which the ReadSelfAction execution is taking place.
    <p>From package UML::Actions.</p>"""
...
class ReduceAction(_user_module.ReduceActionMixin, Action):
    ...
    collection = EReference(ordered=False, unique=True, containment=True, derived=False)
    reducer = EReference(ordered=False, unique=True, containment=False, derived=False)
    result = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ReplyAction(_user_module.ReplyActionMixin, Action):
    ...
    replyToCall = EReference(
        ordered=False, unique=True, containment=False, derived=False
    )
    replyValue = EReference(
        ordered=True, unique=True, containment=True, derived=False, upper=-1
    )
...
class StartClassifierBehaviorAction(
    ...
    """A StartClassifierBehaviorAction is an Action that starts the
    classifierBehavior of the input object.
    <p>From package UML::Actions.</p>"""
...
class TestIdentityAction(_user_module.TestIdentityActionMixin, Action):
    ...
    first = EReference(ordered=False, unique=True, containment=True, derived=False)
    result = EReference(ordered=False, unique=True, containment=True, derived=False)
    second = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ActivityParameterNode(_user_module.ActivityParameterNodeMixin, ObjectNode):
    ...
    parameter = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class CentralBufferNode(_user_module.CentralBufferNodeMixin, ObjectNode):
    """A CentralBufferNode is an ObjectNode for managing flows from multiple
    sources and targets.
    <p>From package UML::Activities.</p>"""
...
class LiteralInteger(_user_module.LiteralIntegerMixin, LiteralSpecification):
    ...
    value = EAttribute(eType=Integer, derived=False, changeable=True, default_value=0)
...
class LiteralUnlimitedNatural(
    ...
    value = EAttribute(
        eType=UnlimitedNatural, derived=False, changeable=True, default_value=0
    )
...
class Classifier(
    ...
    feature = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedFeature,
    )
    attribute = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedAttribute,
    )
    ...
    general = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedGeneral,
    )
    generalization = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    ...
    inheritedMember = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedInheritedmember,
    )
...
class Operation(
    ...
    other behavioral responses. Operation specializes TemplateableElement in order
    to support specification of template operations and bound operations. Operation
    ...
    _isOrdered = EAttribute(
        eType=Boolean, derived=True, changeable=False, name="isOrdered", transient=True
    )
    isQuery = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    ...
    class_ = EReference(ordered=False, unique=True, containment=False, derived=False)
    datatype = EReference(ordered=False, unique=True, containment=False, derived=False)
    interface = EReference(ordered=False, unique=True, containment=False, derived=False)
    postcondition = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
    precondition = EReference(
...
class Realization(_user_module.RealizationMixin, Abstraction):
    ...
    model Elements, one representing a specification (the supplier) and the other
...
@abstract
class WriteVariableAction(_user_module.WriteVariableActionMixin, VariableAction):
...
class AcceptCallAction(_user_module.AcceptCallActionMixin, AcceptEventAction):
    ...
    information to the ReplyAction necessary to return control to the caller.
    An AcceptCallAction is for synchronous calls. If it is used to handle an
...
class BroadcastSignalAction(_user_module.BroadcastSignalActionMixin, InvocationAction):
    ...
    the Signal. The requestor continues execution immediately after the
    Signal instances are sent out and cannot receive reply values.
    <p>From package UML::Actions.</p>"""
...
class CallAction(_user_module.CallActionMixin, InvocationAction):
    ...
    given argument values and (if the invocation is synchronous) receive reply values.
    <p>From package UML::Actions.</p>"""
...
class ClearVariableAction(_user_module.ClearVariableActionMixin, VariableAction):
...
class SendObjectAction(_user_module.SendObjectActionMixin, InvocationAction):
    ...
    object. The requestor continues execution immediately after the object is
    sent out and cannot receive reply values.
    <p>From package UML::Actions.</p>"""

    request = EReference(ordered=False, unique=True, containment=True, derived=False)
    target = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class BehavioredClassifier(_user_module.BehavioredClassifierMixin, Classifier):
    ...
    Behaviors one of which may specify the behavior of the BehavioredClassifier itself.
    <p>From package UML::SimpleClassifiers.</p>"""
...
class Interface(_user_module.InterfaceMixin, Classifier):
    ...
    protocol = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class Signal(_user_module.SignalMixin, Classifier):
    ...
    in which a reaction is asynchronously triggered in the receiver without a reply.
    <p>From package UML::SimpleClassifiers.</p>"""
...
class StructuredClassifier(_user_module.StructuredClassifierMixin, Classifier):
    ...
    part = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedPart,
    )
    role = EReference(
        ordered=False,
        unique=True,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedRole,
    )
...
class Substitution(_user_module.SubstitutionMixin, Realization):
    ...
    are runtime substitutable where instances of the contract classifier are expected.
    <p>From package UML::Classification.</p>"""

    contract = EReference(ordered=False, unique=True, containment=False, derived=False)
...
class InterfaceRealization(_user_module.InterfaceRealizationMixin, Realization):
    ...
    the realizing BehavioredClassifier conforms to the contract specified by the
    Interface.
    <p>From package UML::SimpleClassifiers.</p>"""
...
class StructuredActivityNode(
    _user_module.StructuredActivityNodeMixin, Action, Namespace, ActivityGroup
):
...
class AddStructuralFeatureValueAction(
    _user_module.AddStructuralFeatureValueActionMixin, WriteStructuralFeatureAction
):
    ...
    insertAt = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class CallBehaviorAction(_user_module.CallBehaviorActionMixin, CallAction):
    ...
    The argument values of the CallBehaviorAction are passed on the input
    Parameters of the invoked Behavior. If the call is synchronous, the
    execution of the CallBehaviorAction waits until the execution of the
    invoked Behavior completes and the values of output Parameters of the
    Behavior are placed on the result OutputPins. If the call is asynchronous,
    the CallBehaviorAction completes immediately and no results values can be provided.
    <p>From package UML::Actions.</p>"""
...
class CallOperationAction(_user_module.CallOperationActionMixin, CallAction):
    ...
    of the CallOperationAction waits until the execution of the invoked
    Operation completes and the values of output Parameters of the Operation
    are placed on the result OutputPins. If the call is asynchronous, the
...
class RemoveStructuralFeatureValueAction(
    _user_module.RemoveStructuralFeatureValueActionMixin, WriteStructuralFeatureAction
):
    ...
    removeAt = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class StartObjectBehaviorAction(
    ...
    """A StartObjectBehaviorAction is an InvocationAction that starts the
    execution either of a directly instantiated Behavior or of the classifierBehavior
    of an object. Argument values may be supplied for the input Parameters of the
...
class InformationItem(_user_module.InformationItemMixin, Classifier):
    ...
    information that may move within a system, but do not elaborate details of
    the transferred information.  Details of transferred information are the
    province of other Classifiers that may ultimately define InformationItems.
    Consequently, InformationItems cannot be instantiated and do not themselves
    have features, generalizations, or associations. An important use of
...
class ComponentRealization(_user_module.ComponentRealizationMixin, Realization):
...
class Association(_user_module.AssociationMixin, Classifier, Relationship):
    ...
    endType = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedEndtype,
    )
    memberEnd = EReference(
        ordered=True, unique=True, containment=False, derived=False, upper=-1
    )
    ownedEnd = EReference(
        ordered=True, unique=True, containment=True, derived=False, upper=-1
...
class Property(
    _user_module.PropertyMixin, StructuralFeature, ConnectableElement, DeploymentTarget
):
    """A Property is a StructuralFeature. A Property related by ownedAttribute to
    a Classifier (other than an association) represents an attribute and might
    also represent an association end. It relates an instance of the Classifier
    to a value or set of values of the type of the attribute. A Property related
    by memberEnd to an Association represents an end of the Association. The type
    of the Property is the type of the end of the Association. A Property has the
...
class UseCase(_user_module.UseCaseMixin, BehavioredClassifier):
    ...
    yields an observable result that is of value for one or more Actors or other
...
class EncapsulatedClassifier(
    ...
    ownedPort = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedOwnedport,
    )
...
class ActionInputPin(_user_module.ActionInputPinMixin, InputPin):
    ...
    fromAction = EReference(ordered=False, unique=True, containment=True, derived=False)
...
class ConditionalNode(_user_module.ConditionalNodeMixin, StructuredActivityNode):
...
class ExpansionRegion(_user_module.ExpansionRegionMixin, StructuredActivityNode):
...
class DeploymentSpecification(_user_module.DeploymentSpecificationMixin, Artifact):
    ...
    deploymentLocation = EAttribute(eType=String, derived=False, changeable=True)
    executionLocation = EAttribute(eType=String, derived=False, changeable=True)
    deployment = EReference(
        ordered=False, unique=True, containment=False, derived=False
    )
...
class Port(_user_module.PortMixin, Property):
    ...
    isBehavior = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isConjugated = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=False
    )
    isService = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=True
    )
    protocol = EReference(ordered=False, unique=True, containment=False, derived=False)
    provided = EReference(
        ordered=False,
        unique=True,
        containment=False,
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedProvided,
    )
    ...
    required = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedRequired,
    )
...
class Extension(_user_module.ExtensionMixin, Association):
    ...
    extended through a stereotype, and gives the ability to flexibly add
    (and later remove) stereotypes to classes.
    <p>From package UML::Packages.</p>"""

    _isRequired = EAttribute(
        eType=Boolean, derived=True, changeable=False, name="isRequired", transient=True
    )
...
class Collaboration(
    _user_module.CollaborationMixin, StructuredClassifier, BehavioredClassifier
):
...
class Class(_user_module.ClassMixin, EncapsulatedClassifier, BehavioredClassifier):
    """A Class classifies a set of objects and specifies the features that
    characterize the structure and behavior of those objects.  A Class may
    have an internal structure and Ports.
    <p>From package UML::StructuredClassifiers.</p>"""
    ...
    extension = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedExtension,
    )
    nestedClassifier = EReference(
        ordered=True, unique=True, containment=True, derived=False, upper=-1
    )
    ...
    superClass = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedSuperclass,
    )
...
class Behavior(_user_module.BehaviorMixin, Class):
    """Behavior is a specification of how its context BehavioredClassifier
    changes state over time. This specification may be either a definition
    of possible behavior execution or emergent behavior, or a selective
    illustration of an interesting subset of possible executions. The latter
    form is typically used for capturing examples, such as a trace of a
    particular execution.
    <p>From package UML::CommonBehavior.</p>"""

    isReentrant = EAttribute(
        eType=Boolean, derived=False, changeable=True, default_value=True
    )
    specification = EReference(
        ordered=False, unique=True, containment=False, derived=False
...
class Activity(_user_module.ActivityMixin, Behavior):
    ...
    coordinated sequencing of subordinate units.
    <p>From package UML::Activities.</p>"""
    ...
    node = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedNode,
    )
    variable = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    group = EReference(
        ...
        upper=-1,
        transient=True,
        derived_class=_user_module.DerivedGroup,
    )
    ownedNode = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
    partition = EReference(
        ordered=False, unique=True, containment=False, derived=False, upper=-1
    )
...
class StateMachine(_user_module.StateMachineMixin, Behavior):
    ...
    interconnected by one or more joined Transition arcs that are triggered
    by the dispatching of successive Event occurrences. During this traversal,
    the StateMachine may execute a sequence of Behaviors associated with various
...
class Node(_user_module.NodeMixin, Class, DeploymentTarget):
    ...
    for execution. Nodes can be interconnected through communication paths to
    define network structures.
    <p>From package UML::Deployments.</p>"""

    nestedNode = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
...
class ProtocolStateMachine(_user_module.ProtocolStateMachineMixin, StateMachine):
    ...
    which State and under which conditions, thus specifying the allowed
    invocation sequences on the Classifier's BehavioralFeatures. A
    rotocolStateMachine specifies the possible and permitted Transitions on
    the instances of its context Classifier, together with the BehavioralFeatures
    that carry the Transitions. In this manner, an instance lifecycle can be
    specified for a Classifier, by defining the order in which the BehavioralFeatures
    can be activated and the States through which an instance progresses during
    its existence.
    <p>From package UML::StateMachines.</p>"""

    conformance = EReference(
        ordered=False, unique=True, containment=True, derived=False, upper=-1
    )
...
class Interaction(_user_module.InteractionMixin, Behavior, InteractionFragment):
    """
...
class AssociationClass(_user_module.AssociationClassMixin, Class, Association):
    ...
    AssociationClass can be seen as an Association that also has Class
    properties, or as a Class that also has Association properties. It not
    only connects a set of Classifiers but also defines a set of Features
    that belong to the Association itself and not to any of the associated Classifiers.
    <p>From package UML::StructuredClassifiers.</p>
    """
